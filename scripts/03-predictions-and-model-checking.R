...
# 1. Draw the estimated smooth again, this time using `seWithMean = TRUE`. Does
# it change the plot much? Think why this is the case.
plot(rich_depthyear, pages=1,seWithMean = TRUE)
# 2. Using `draw()`, draw the estimated smooth

# 3. Repeat 2 but this time account for the additional uncertainty due to
# estimating the smoothness parameters using `unconditional = TRUE`


plot(rich_depthyear, pages=1,seWithMean = TRUE,unconditional = TRUE)
...
sm_draws_delta <- Xp_delta %*% t(beta_sim)
dim(sm_draws)
